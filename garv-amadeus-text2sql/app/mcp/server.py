...
def _auth_ok(request: Request, gov_cfg: Dict[str, Any]) -> bool:
    ...
    expected = (os.getenv("MCP_API_KEY", "") or "").strip()  # secret stays in env
    if not expected:
        # if auth enabled but key missing, deny
        return False
...
def run_sql(sql: str, limit: int = 50) -> Dict[str, Any]:
    # MCP tool calls (no Request object). Keep tool simple for now.
    # For your app you are using bridge endpoints, so governance is enforced there.
    conn = get_conn()
...
async def bridge_run_sql(request: Request):
    ...
    if not _auth_ok(request, gov):
        if audit_enabled:
            _audit_log(audit_path, {"ts": _now_iso(), "event": "bridge_run_sql", "status": "DENY", "reason": "bad_api_key", "request_id": req_id, "role": role})
        return JSONResponse({"error": "Unauthorized"}, status_code=401)

    try:
        body = await request.json()
        sql = str(body.get("sql", "")).strip()
        limit = int(body.get("limit", (gov.get("limits", {}) or {}).get("default_limit", 50)))
